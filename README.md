...
> For more details about the "config reload" application -- how it works and what assumption it makes, see [`./docker/README.md`](https://gitlab.com/nvidia-networking/systems-engineering/poc-support/kubernetes-on-baremetal-network/-/blob/main/docker/README.md).
...
All devices start with default configuration, management IPs are allocated by the DHCP server running on the `OOB` node.

> To learn how to install kubelet on Cumulus and SONiC and taint the nodes to prevent them from running unwanted containers, see the [`./air`](https://gitlab.com/nvidia-networking/systems-engineering/poc-support/kubernetes-on-baremetal-network/-/tree/main/air) directory.
...
If flux was able to succesfully pull the specified repository, it will update the status of the resource to `READY`:
...
Now we need to tell flux where in the repository should it look for the Kubernetes manifests to reconcile. Reconciliation process is done by the kustomization controller so we need to create a resource that will:

* Reference the previsouly created git repository
